            ...
            steps {
                script {
                    ...
                    withCredentials([
                        ...
                        withCredentials([usernamePassword(credentialsId: "${ACR_CREDS_ID}", usernameVariable: 'ACR_USER', passwordVariable: 'ACR_PASS')]) {
                            runCmd "ssh -i \"${SSH_KEY}\" -o StrictHostKeyChecking=no ${SSH_USER}@${AZURE_VM_PRIVATE_IP} \"docker login ${ACR_REGISTRY} -u ${ACR_USER} -p ${ACR_PASS} && cd /home/${SSH_USER}/campus-find && docker compose pull && docker compose up -d --remove-orphans && sleep 3 && docker compose logs backend && docker image prune -f\""
                        }
                    }
                }
            }
